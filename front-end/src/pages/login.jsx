...
const LoginSection = () => {
    ...
    return (
        <section className="text-white w-full md:h-screen p-3 bg-gradient-to-r from-[#296ED3] to-[#3ED0EA] flex flex-col gap-y-5 md:gap-y-44 items-center">
            <header className="h-28 w-full flex justify-between items-center px-5 lg:justify-around">
            ...
            <main className="w-full lg:w-2/3 flex flex-col gap-y-14 md:gap-y-48 lg:flex-row-reverse justify-center items-center lg:justify-between">
                ...
                <section className="w-full flex flex-col justify-center items-center">
                    <div className="flex flex-col gap-y-3 w-full p-3">
                        <h1 className="text-center text-4xl font-bold md:text-5xl">Welcome Back</h1>
                        <div className="flex gap-x-1 justify-center">
                            <p className="text-lg md:text-3xl">Dont Have An Account?</p>
                            <Link to="/register">
                                <span className="text-[#DE4FC1] text-lg md:text-3xl">Register</span>
                            </Link>
                        </div>
                        <form onSubmit={handleLogin} className="flex flex-col items-center gap-y-5">
                            ...
                            <button type="submit" className="p-3 bg-[#DE4FC1] text-white rounded-lg w-56 md:w-80 md:text-xl">Login</button>
                        </form>
                    </div>
                </section>
            </main>
            ...
            {popup.isVisible && (
                <div className={`fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-50 z-50 p-5`}>
                    <div className="bg-white md:w-1/2 lg:w-1/3 p-5 rounded-lg shadow-lg text-center w-full flex flex-col gap-y-5">
                        {popup.isSuccess ? (
                            <h1 className="text-black font-bold">Selamat Datang Tuan {dataLogin.username}</h1>
                        ) : (
                            <h1 className="text-black font-bold">Maaf Tuan {dataLogin.username}, Anda Tidak Bisa Login</h1>
                        )}
